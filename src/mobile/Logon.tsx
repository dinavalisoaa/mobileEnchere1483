import { IonContent, IonPage, IonTitle, IonButton, IonInput, IonItem, IonLabel, IonCol, IonIcon, IonGrid, IonCard } from '@ionic/react';
...
const Logon: React.FC = () => {
  ...
  const [passwordRetap, setPasswordRetap] = useState('');
  ...
  function log() {
    ...
    console.log(URL_IP()+"/inscription?nom="+nom+"&prenom="+prenom+"&dtn=2019-11-11&logins="+login+"&pwds="+password);
    fors.append("nom", nom);
    fors.append("prenom", prenom);
    fors.append("dtn", dtn);

    fors.append("logins", login);
    fors.append("pwds", password);

    fetch(URL_IP()+"/inscription", {
      method: 'POST',
      body: fors
    }).then((result) => {
      return result.json();
  ...
  function verif() {
      setError("");
      if(password!=""){
    if (password != passwordRetap) {
      setError("Retaper le bon mot de passe");
      setColor("red");
    } else {
      setError("OK");
      setColor("green");
      ...
      <form onSubmit={handleSubmit(registerUser)} id="forme">
        <IonGrid>
          ...
          <IonCol>
            ...
            <IonItem>
              <IonInput placeholder='password' value={passwordRetap} type="password" onIonChange={(e: any) => {
                setPasswordRetap(e.target.value); 
              }} ></IonInput>
              <IonLabel style={{"color":{color}}}>{error}</IonLabel>
            </IonItem>
            ...
            <Link to={"/login"}  >    <IonButton color="danger"  >Login
